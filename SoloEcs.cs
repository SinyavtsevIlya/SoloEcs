...
namespace SoloEcs
{
    ...
    public class World
    {
        ...
        public World(int capacity)
        {
            ...
            Worlds.WorldsPools[Index] = new ResizableArray<PoolBase>(Worlds.MaxComponentsCount);
        ...
        public void Destroy(Entity entity)
        {
            ...
            {
                Worlds.WorldsPools[Index].Values[entity.Composition.Dense[i]].DeactivateAtIndexBase(entity.Id);
            }
    ...
    {
        public static int MaxWorldsCount = 8;
        ...
        static int _worldsCount;
        static int[] _pooled = new int[64];
        static int _pooledCount;

        public static int Pool()
        {
            if (_worldsCount > Worlds.MaxWorldsCount) throw new Exception($"Unable to create more than {Worlds.MaxWorldsCount} worlds. Specify Worlds.MaxWorldsCount at startup.");
            return _pooledCount > 0 ? _pooled[--_pooledCount] : _worldsCount++;
        }

        public static void Push(int worldIndex)
        {
            if (_pooled.Length == _pooledCount)
            {
                Array.Resize(ref _pooled, _pooledCount << 1);
            }
            _pooled[_pooledCount++] = worldIndex;
        }
    }
    ...
    internal class ComponentLayer<T> where T : struct
    {
        ...
        {      
            WorldPools = new Pool<T>[Worlds.MaxWorldsCount];
            Indecies = new byte[Worlds.MaxWorldsCount];
            for (int i = 0; i < Worlds.MaxWorldsCount; i++) 
            {
                Indecies[i] = 255;
            }
        }
        ...
        internal static void TryRegister(int worldIndex, int capacity)
        {
            ...
            {
                Indecies[worldIndex] = Convert.ToByte(Worlds.TypeIndeciesCounts[worldIndex]++);
                ...
                Singleton = pool;
#endif
                Worlds.WorldsPools[worldIndex].Add(pool);
            }
        }
    }
...
Repeat:
        ...
        {
            var pool = Worlds.WorldsPools[_world.Index].Values[componentIndex];
            HandleWith(pool);
            return this;
        }
